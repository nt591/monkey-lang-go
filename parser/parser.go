...
func New(l *lexer.Lexer) *Parser {
	...
	// read two tokens, so we have cur and peek tokens
	p.nextToken()
	p.nextToken()
...
// ParseProgram entry point - create a program root node and begin parsing tokens
// For each token, if it's not an EOF, figure out what kind of statement it is and add to the list
...
func (p *Parser) parseBlockStatement() *ast.BlockStatement {
	...
	// go until we see a closing brace or the end of the file
...
func (p *Parser) parseHashLiteral() ast.Expression {
	...
	for !p.peekTokenIs(token.RBRACE) {
		...
		// if the next token isn't either a comma or closing brace, no hash literal
